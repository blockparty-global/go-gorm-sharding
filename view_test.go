...
func TestCreateHoldersViewWithPartitioning(t *testing.T) {
	...
	t.Run("GetHoldersCount", func(t *testing.T) {
		...
		assert.Equal(t, int64(2), count, "Query should return 5 distinct account+owner combinations")
	...
	t.Run("CreateAndVerifyView", func(t *testing.T) {
		...
		assert.Equal(t, int64(2), viewCount, "View should return 5 distinct account+owner combinations")
	})
	...
	t.Run("DirectModelQuery", func(t *testing.T) {
		...
		assert.Equal(t, int64(2), directCount, "Direct query should return 5 distinct account+owner combinations")
	})
...
// TestCreateOrReplaceView tests that CREATE OR REPLACE VIEW statements work correctly
func TestCreateOrReplaceView(t *testing.T) {
	...
	// Test: Create a view using CREATE OR REPLACE VIEW
	t.Run("CreateOrReplaceView", func(t *testing.T) {
		// Create the view using CREATE OR REPLACE VIEW
		viewQuery := `
			CREATE OR REPLACE VIEW "holders_0xcontract4" AS 
			SELECT COUNT(DISTINCT(account || owner)) 
			FROM "generic_balances" 
			WHERE contract = '0xcontract4' AND (type = 'ERC721' OR (value IS NOT NULL AND value > 0))
		`
		err := testDB.Exec(viewQuery).Error
		assert.NoError(t, err, "CREATE OR REPLACE VIEW should work correctly")
