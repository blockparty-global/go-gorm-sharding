...
// TestSelectDoubleWriteDisabled tests the same scenario but with DoubleWrite disabled
// This should fail when querying by ID without the sharding key
func TestSelectDoubleWriteDisabled(t *testing.T) {
	// Create a test DB connection
	testDB, err := gorm.Open(postgres.New(dbConfig), &gorm.Config{
		DisableForeignKeyConstraintWhenMigrating: true,
		Logger:                                   logger.Default.LogMode(logger.Info),
	...
	// Configure sharding where the sharding key is user_id, NOT id, and DoubleWrite is DISABLED
	imageShardingConfig := Config{
		DoubleWrite:         false,     // Disable double write
		ShardingKey:         "user_id", // Sharding key is user_id, not id
		NumberOfShards:      4,
		PrimaryKeyGenerator: PKSnowflake,
		PartitionType:       PartitionTypeHash,
	...
	// Test Case: Query by ID without sharding key should fail when DoubleWrite is disabled
	t.Run("SelectByIDWithoutDoubleWrite", func(t *testing.T) {
		var foundImage Image

		// This should fail because DoubleWrite is disabled and we're not providing the sharding key
		err := testDB.Model(&Image{}).Where("id = ?", 1001).First(&foundImage).Error

		// Should fail with ErrMissingShardingKey
		require.Error(t, err, "Query by ID should fail when DoubleWrite is disabled")
		require.Equal(t, ErrMissingShardingKey, err, "Should return missing sharding key error")
	})
