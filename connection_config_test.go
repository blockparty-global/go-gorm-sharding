...
func TestConnectionConfigDefaults(t *testing.T) {
	...
	assert.Equal(t, 3600, config.Connection.MaxLifetime, "Default MaxLifetime should be 3600 seconds (1 hour)")
	assert.Equal(t, 300, config.Connection.TransactionTimeout, "Default TransactionTimeout should be 300 seconds (5 minutes)")
