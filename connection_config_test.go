...
func TestConnectionConfigDefaults(t *testing.T) {
	...
	assert.Equal(t, 3, config.Connection.MaxLifetime, "Default MaxLifetime should be 3 seconds")
	assert.Equal(t, 3, config.Connection.TransactionTimeout, "Default TransactionTimeout should be 3 seconds")
